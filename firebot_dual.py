"""
firebot_dual.py — Fire-detection bot with staggered extinguisher trigger

Drives forward continuously.
Fire detected → stops immediately.
Fire within 0.5m for 1.3s → triggers servo 1 (extinguisher 1).
Fire within 0.5m for 2.0s → triggers servo 2 (extinguisher 2).

Arduino sketch: pbot_dual.ino

Usage:
    python firebot_dual.py
"""
...
SERVO1_HOLD_SEC     = 1.3     # servo 1 fires after 1.3s
SERVO2_HOLD_SEC     = 2.0     # servo 2 fires after 2.0s
...
try:
    while True:
        ...
        # ── Staggered extinguisher timer ──────────────────────────────────
        closest = closest_fire_distance(fire_boxes, depth_frame, fw, fh)
        now = time.monotonic()

        if closest is not None and closest <= EXTINGUISH_DIST_M:
            # Fire is within range
            ...
            # Fire left range — reset timer (but don't un-fire servos)
            if close_fire_start is not None and not servo1_fired:
