"""
firebot_extinguisher.py — Fire-detection bot + extinguisher trigger

Drives forward continuously.
Fire detected → stops immediately.
Fire within 0.5m for 3 consecutive seconds → triggers both servos (extinguishers).

Arduino sketch: pbot_extinguisher.ino

Usage:
    python firebot_extinguisher.py
"""
...
EXTINGUISH_DIST_M   = 0.5
EXTINGUISH_HOLD_SEC = 3.0
...
def send_cmd(cmd: str):
    if arduino is None or cmd not in ("F", "S", "E"):
        return
...
def closest_fire_distance(boxes, depth_frame, fw, fh):
    closest = None
    for box in boxes:
        ...
        if dist > 0 and math.isfinite(dist):
            ...
            if cy >= fh // 2:
                if closest is None or 0.1 < closest:
                    closest = 0.1
...
print("[firebot]   Drives forward → stops on fire", flush=True)
print(f"[firebot]   Fire within {EXTINGUISH_DIST_M}m for {EXTINGUISH_HOLD_SEC}s → extinguishers fire", flush=True)
print("[firebot]   Press 'q' in the video window to quit.\n", flush=True)

close_fire_start = None
extinguished     = False
...
try:
    while True:
        ...
        # ── Extinguisher timer ────────────────────────────────────────────
        closest = closest_fire_distance(fire_boxes, depth_frame, fw, fh)
        now = time.monotonic()

        if not extinguished and closest is not None and closest <= EXTINGUISH_DIST_M:
            if close_fire_start is None:
                close_fire_start = now
                print(f"[firebot] Fire within {EXTINGUISH_DIST_M}m — timer started", flush=True)

            elapsed = now - close_fire_start
            remaining = EXTINGUISH_HOLD_SEC - elapsed

            if elapsed >= EXTINGUISH_HOLD_SEC:
                extinguished = True
                send_cmd("E")
                print("[firebot] >>> EXTINGUISHERS TRIGGERED!", flush=True)
            else:
                cv2.putText(frame, f"EXTINGUISH IN: {remaining:.1f}s", (10, fh-50),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)
        else:
            if close_fire_start is not None and not extinguished:
                print("[firebot] Fire left range — timer reset", flush=True)
            close_fire_start = None

        # ── Motor + status ────────────────────────────────────────────────
        if extinguished:
            send_cmd("E")
            cv2.putText(frame, "EXTINGUISHERS ACTIVE", (10, fh-16),
