/*
 * pbot_extinguisher.ino — Fire bot + servo extinguisher trigger
 * Arduino Mega  •  9600 baud  •  Osoyoo Mecanum
 *
 * Commands from firebot_extinguisher.py:
 *   'F' → drive forward (all 4 wheels)
 *   'S' → stop all motors
 *   'E' → stop + trigger extinguishers (both servos to 180°)
 *
 * Servos:
 *   D3 → extinguisher 1
 *   D4 → extinguisher 2
...
void trigger_extinguishers() {
  if (!extinguished) {
    Serial.println(F(">> EXTINGUISH: servos to 180"));
    servo1.write(180);
    servo2.write(180);
    extinguished = true;
  }
...
void setup() {
  ...
  Serial.begin(9600);
  Serial.println(F("pbot ready. F=forward S=stop E=extinguish"));
  lastCmdTime = millis();
}
...
void loop() {
  while (Serial.available()) {
    ...
    if (c == 'E') { stop_all(); trigger_extinguishers(); lastCmdTime = millis(); }
  }
